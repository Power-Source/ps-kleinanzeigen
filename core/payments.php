...
class CF_Payments{
	...
	*/
	function handle_checkout_requests() {
		...
		global $blog_id, $current_user, $Classifieds_Core;

		// Only handle request if on the proper page
		if ( ! (isset($Classifieds_Core) && is_page($Classifieds_Core->checkout_page_id) ) ) return;
		...
		$this->paypal_options['return_url'] = get_permalink($Classifieds_Core->checkout_page_id);
		...
		if ( isset( $_POST['terms_submit'] )) {
			...
			if ( empty( $_POST['tos_agree'] ) || empty( $_POST['billing_type'] ) ) {
				...
				elseif($_SESSION['billing_type'] == 'one_time') {
					...
					$_SESSION['billing_agreement'] = $options['payments']['one_time_txt'];
				}
		...
		elseif ( isset( $_POST['confirm_payment_submit'] ) ) {
			...
			if( $_SESSION['payment_method'] == 'cc' && $this->use_authorizenet) {
				...
				$invoice_key = uniqid("CLS-{$blog_id}-"); // CLS is the prefix for Gateway Relay
